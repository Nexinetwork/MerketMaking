...
public interface MarketMakingWalletDao extends BaseLongDao<MarketMakingWallet> {
	...
	public List<MarketMakingWallet> findNByContractOrderByRandom(@Param("contract") SmartContract contract,
	...
	public List<MarketMakingWallet> findNWalletsRandomByContractIdNative(@Param("contractId") long contractId,
			@Param("count") int count);
	...
	public List<MarketMakingWallet> findNWalletsRandomByContractIdAndWalletTypeNative(
			@Param("contractId") long contractId, @Param("walletType") int walletType, @Param("count") int count);
	...
	public List<MarketMakingWallet> findAllWalletsByContractIdAndWalletTypeNative(@Param("contractId") long contractId,
			@Param("walletType") int walletType);

	@Query(nativeQuery = true, value = "SELECT \"mmWalletId\",PGP_SYM_DECRYPT( \"privateKey\", '!@MYLoveTeted2023secretLOGINILoveYouTedTed@!' ) as \"privateKey\",PGP_SYM_DECRYPT( \"privateKeyHex\", '!@MYLoveTeted2023secretLOGINILoveYouTedTed@!' ) as \"privateKeyHex\",\"publicKey\",balance,\"mainCoinBalance\",\"contractId\",\"coinId\",\"blockchainId\",\"contractAddress\",\"walletType\",\"creationDate\",\"lastModifiedDate\" FROM \"schMarketMaking\".\"tblMMWallet\" WHERE \"contractId\" = :contractId AND \"walletType\" = :walletType", countQuery = "SELECT COUNT(*) FROM \"schMarketMaking\".\"tblMMWallet\" WHERE \"contractId\" = :contractId AND \"walletType\" = :walletType")
	public Page<MarketMakingWallet> findAllWalletsByContractIdAndWalletTypeNativePaged(@Param("contractId") long contractId,
			@Param("walletType") int walletType, Pageable pageable);
	...
	public List<MarketMakingWallet> findNWalletsRandomByContractIdNativeTABLESAMPLE(
			@Param("contractId") long contractId, @Param("count") int count);
	...
	public List<MarketMakingWallet> findNWalletsRandomByContractIdAndWalletTypeNativeTABLESAMPLE(
			@Param("contractId") long contractId, @Param("walletType") int walletType, @Param("count") int count);
